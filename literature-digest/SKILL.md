...
- `meta`：
  ...
  - `scope`：`{ "section_title": "Introduction", "line_start": <int>, "line_end": <int> }`
- `items`：数组（按被引文献聚合）
...
- `summary`（该被引文献在 Introduction 内的综合总结，1~2 句）
...
3) 使用全文 Markdown 生成“结构化骨架”（建议为 JSON）：包含大纲、Introduction 区块行号范围、以及 references 区块位置（按原始 md 行号 1-based）。
4) 基于骨架：
   - 生成 digest（全局总结 + 分章节总结）
   - 定位 references 区块并抽取条目（结构化引文条目）
   - 在 **Introduction（第一章，含子节）** 范围内做 citation analysis（被引文献聚合 + 分类 + 综合总结）
...
- **必须由 LLM 执行（不得用脚本替代）**：
  ...
  - 基于全文理解生成结构化骨架（大纲、章节范围、introduction及references 区块定位）
  - 参考文献条目的语义级字段推断（作者/标题等）在规则不稳时的补全
  - 基于 Introduction（第一章，含子节）提取并总结文内引文（numeric / author-year），并与 references 做映射（citation analysis）
...
## Citation Analysis（Introduction only）细则

目标：辅助用户撰写文献综述。仅基于论文 **第一章 Introduction（含其子节）** 的内容，整理：
- 本文引用了哪些文献（与 `references.json` 关联）
- 本文在 Introduction 中如何介绍/定位这些被引工作（背景/基线/对比/组件等）

### 范围硬约束

- **只分析 Introduction 范围内出现的引文**：从 `Introduction` 标题行开始，到下一个“同级或更高等级标题”之前结束（含其子节）。
- 引用出现在其他章节（Methods/Experiments/Appendix/References 等）的一律不纳入本产物。
- 输出 `meta.scope.line_start/line_end`（1-based，按原始 md 行号）。

### 支持的引文体例（必须同时支持）

1) numeric（编号型）：
- 支持：`[5, 36]`、`[4,15,38]`、`[40–42]`（需要展开为 40/41/42）
- 映射：优先 `ref_number -> ref_index`（通常 `ref_index = ref_number - 1`）；若参考文献编号异常（例如 raw 中出现 `3937.`），以 `references.json` 实际顺序为准并降低 `confidence`，不强行猜。

2) author-year（作者-年份型，高质量要求）：
- 必须识别：`(Smith, 2020)`、`Smith et al. (2020)`、`(Smith & Jones, 2020; Brown, 2019)` 等
- 映射规则（禁止硬猜）：
  - 优先在 `references.json` 中按 `year` + “first-author surname（或作者字符串中最显著的姓）”匹配。
  - 必要时可结合 `raw`（参考文献原文）做二次判别。
  - 若无法可靠映射：写入 `unmapped_mentions`，并给出 `reason`（`no_match/ambiguous/parse_failed` 等）。
...
- 为避免 stdout 截断，必须将结果文件直接写入**输入文档 `md_path` 所在目录**（便于受限 agent 访问）：
  ...
  - `citation_analysis_path=<dir_of_md_path>/citation_analysis.json`

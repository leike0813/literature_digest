...
- 正式发布路径一律从 `runtime_inputs.output_dir` 读取目标目录
- `literature-digest.result.json` 一律写到 `runtime_inputs.result_json_path`
- digest Markdown 与 `citation_analysis.md` 一律从 DB 已固化的运行时模板路径读取：
  - `runtime_inputs.digest_template_path`
  - `runtime_inputs.citation_analysis_template_path`
- 最终 stdout JSON 中所有文件路径字段一律输出为绝对路径
- 若旧库缺失这些目录键，脚本只在兼容模式下回退到当前工作目录
- 文件名固定：`digest.md`、`references.json`、`citation_analysis.json`、`literature_matching_metadata.json`（UTF‑8）。
- render 还会把最终 stdout JSON 同步写入 DB 中 `runtime_inputs.result_json_path` 指向的固定文件
- 若 digest 阶段持久化了 `representative_image`，render 会把该对象纳入最终 stdout JSON 与 `literature-digest.result.json`；未持久化时不输出该可选字段。
...
- 在校验失败时做“可解释的自动修复”（例如补齐缺失字段、类型纠正、将过长输出物化到文件），并将修复记录写入 `warnings`
...
- `literature_matching_metadata.json` 必须由 stage 2 写入 DB 的 `literature_matching_metadata` 渲染
- `representative_image` 不生成公开 artifact；它只作为最终 stdout JSON 的可选元数据字段
- `--mode render` 不得再接受显式 `source_path`、`preprocess_artifact`、stdin payload 或输出目录覆盖；这些输入必须来自 DB
- `--mode render` 结束时必须把同一个 stdout JSON 再写入 `runtime_inputs.result_json_path`
- 新主路径下，render 不再直接读取仓库 `assets/templates` 中的 digest / citation Markdown 模板；它们只作为 `persist_render_templates` 的 source template
...
- `literature_matching_metadata.json.schema` 必须为 `literature_matching_metadata.v1`
- `representative_image` 若存在，必须来自 digest 阶段持久化的结构化选择结果

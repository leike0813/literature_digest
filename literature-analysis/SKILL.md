...
8. 若环境支持 subagent，reference review、metadata review、citation semantic review 优先按 batch 委派；只有主智能体可以合并并提交一次正式 payload。
...
- `split_reviews[*]` 字段含义：
  ...
  - `corrected_reference_texts`：当需要修正条目边界时，给出修正后的完整 reference 文本数组；边界改变后必须使用 runtime 重新生成的 packages 再做 reference review。
- Subagent hard rules：
  - 若环境支持 subagent，core reference review 和 metadata review 都优先按 runtime 返回的 batch packages 分批委派。
...
- Subagent hard rules：
  - 若环境支持 subagent，优先按 `batch_work_packages` 委派 citation semantic review。

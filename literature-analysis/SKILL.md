...
8. 当环境支持 subagent 且 runtime 返回可分批的 `batch_work_packages` 时，reference review、metadata review、citation semantic review 必须默认按 batch 委派；只有主智能体可以合并并提交一次正式 payload。
...
Reference core review、reference metadata enrichment、citation semantic review 是本 skill 的默认可委派工作。只要当前环境支持 subagent、runtime/JIT 返回 `batch_work_packages`，且 batch 不是小到无需拆分或上下文不可拆，主 agent 必须默认按 batch 委派。跳过委派时，主 agent 应在执行 notes、`review_notes` 或 payload 合并说明中保留原因。

主 agent 职责：

- 读取 runtime/JIT 返回的 work packages、allowed enums、field guidance、batch prompts 和 merge contract。
- 按 `batch_work_packages` 委派 subagent，收集 batch draft。
...
- `batch_work_packages`：reference/citation workset 的批次边界，供 subagent 草拟。
...
- `reference_reviews[*]` 字段含义：
  - `reference_key`：来自 `reference_review_packages[*].reference_key` 的稳定工作键。
  - `selected_parse_pattern`：必填 parse hypothesis；只能取同一 `reference_key` 在 `allowed_parse_patterns_by_reference_key` 中列出的值。
...
- `reference_reviews[*]` 禁止字段：
  ...
  - 富元数据必须等 runtime 返回 `metadata_review_packages` 后，通过 `metadata_reviews[]` 单独提交。
- `metadata_reviews[*]` 字段含义：
  - `reference_key`：来自 `metadata_review_packages[*].reference_key`。
...
- `split_reviews[*]` 字段含义：
  - `block_key`：来自 `split_review_packages[*].block_key`。
...
- Subagent hard rules：
  - 当环境支持 subagent 且存在 `batch_work_packages` 时，core reference review 和 metadata review 必须默认按 runtime 返回的 batch packages 分批委派。
  - 只有环境不支持、batch 极小，或上下文不可拆时才由主 agent 自行完成；跳过原因写入执行 notes 或 `review_notes`。
  - subagent 只返回 batch draft；不得写 DB、不得改 key、不得补 `raw`/`confidence`/`ref_index` 等内部审计字段。
  - 主 agent 先合并 core reference drafts 并提交 `reference_reviews[]`；成功后读取 `metadata_review_packages`，再合并 metadata drafts 并提交 `metadata_reviews[]`。
  - 主 agent 是唯一 DB 写入者。
...
Use only the provided metadata_review_packages, instructions.allowed_metadata_fields, and instructions.locked_fields.
...
- `citation_semantic_reviews[*]` 字段含义：
  - `citation_work_key`：来自 `citation_work_packages[*].citation_work_key` 的稳定工作键。
...
- Subagent hard rules：
  - 当环境支持 subagent 且存在 `batch_work_packages` 时，citation semantic review 必须默认按 batch 委派。
  - 只有环境不支持、batch 极小，或上下文不可拆时才由主 agent 自行完成；跳过原因写入执行 notes。
  - subagent 只返回 `citation_semantic_reviews[]` draft；主 agent 合并、去重、补齐覆盖后提交一次正式 payload。
...
```
- Citation hard rules：
  - 语义阶段只消费 `citation_work_packages`，不得重新全文盲扫或重做 mention-reference join。

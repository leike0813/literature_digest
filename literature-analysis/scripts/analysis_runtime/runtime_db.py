...
def fetch_reference_metadata_enrichment_workset(connection: sqlite3.Connection) -> list[dict[str, Any]]:
    ...
    for row in rows:
        workset.append(
            {
                "ref_index": int(row["ref_index"]),
                "reference_key": f"reference-{int(row['ref_index'])}",
...
REFERENCE_PUBLIC_METADATA_FIELDS = {
    "publicationTitle",
    "conferenceName",
    "archiveID",
    "university",
...
REFERENCE_AUDIT_METADATA_FIELDS = {"entry_index", "selected_pattern", "pattern_candidate"}


def _public_reference_item(item: dict[str, Any]) -> dict[str, Any]:
    public_item = {
        "ref_index": int(item["ref_index"]),
    ...
    for field in REFERENCE_PUBLIC_METADATA_FIELDS:
        if field in item:
...
def build_reference_parse_audit_context(connection: sqlite3.Connection) -> dict[str, Any]:
    ...
    for item in fetch_reference_items(connection):
        ref_index = int(item["ref_index"])
        audit_items.append(
            {
                "ref_index": ref_index,
                "reference_key": f"reference-{int(item.get('entry_index', ref_index))}",
                "selected_parse_pattern": item.get("selected_pattern"),
                "pattern_candidate": item.get("pattern_candidate"),
                "citekey": item.get("citekey"),
                "bibitem_key": item.get("bibitem_key"),
                "confidence": float(item.get("confidence", 0.0)),
                "raw": str(item.get("raw", "")),
                "title": str(item.get("title", "")),
            }
        )
    return {"kind": "reference_parse_audit", "items": audit_items}

...
- 批量场景下必须可校验不串单：响应需要包含 `digest_path`/`references_path`/`citation_analysis_path`，并保证 `md_path`/`language` 调用一致性。
- 本 skill 用于后台自动化：运行过程中**不得询问用户做决策**；遇到分支必须采用文档约定的默认行为并继续执行，保证可批处理。
  ...
  - 为避免 stdout 截断，skill 不直接在 stdout 输出 `digest`/`references`/`citation_analysis` 正文；而是将其写入**输入文档 `md_path` 所在目录**下的固定文件名，并在 stdout JSON 中回传路径。
...
- `md_path` 不存在/无法读取：返回 JSON，填充 `error`，`digest_path=""`，`references_path=""`，`citation_analysis_path=""`。

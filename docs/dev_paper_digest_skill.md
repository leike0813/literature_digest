...
本 skill 用于在本机 `codex exec` 场景下，读取由 Zotero 插件提供的论文源文件（Markdown、PDF 或无扩展名文件），并输出：

1) 中文 digest（Markdown）
2) 参考文献条目列表（结构化 JSON；至少 `author[]/title/year`）
3) 引文分析（结构化 JSON；仅第一章 Introduction，支持 numeric 与 author-year）
...
- 输入只有一个 `source_path`，尽可能稳定地产出：
  - digest：可直接写入 Zotero note
  - references：可用于后续“文献匹配组件”继续做 citekey 匹配/递归入库
- 输出必须是**严格 JSON**（stdout 仅输出 JSON，不夹杂解释/日志）。
- 批量场景下必须可校验不串单：响应需要包含 `digest_path`/`references_path`/`citation_analysis_path`，并保证 `source_path`/`language` 调用一致性。
- 本 skill 用于后台自动化：运行过程中**不得询问用户做决策**；遇到分支必须采用文档约定的默认行为并继续执行，保证可批处理。
  ...
  - 为避免 stdout 截断，skill 不直接在 stdout 输出 `digest`/`references`/`citation_analysis` 正文；而是将其写入**原始输入 `source_path` 所在目录**下的固定文件名，并在 stdout JSON 中回传路径。
...
- `source_path` 不存在/无法读取：返回 JSON，填充 `error`，`digest_path=""`，`references_path=""`，`citation_analysis_path=""`。
...
```json
{
  "source_path": "C:\\\\Zotero\\\\storage\\\\XXXX\\\\paper",
  "language": "zh-CN"
}
```

约束：
- `source_path` 是唯一内容来源；skill 必须自行探测格式并统一生成 `<cwd>/.literature_digest_tmp/source.md` 作为下游唯一 Markdown 输入。
- `language` 支持：
  - `zh-CN`（默认）
  - `en-US`
...
- 输出对象必须包含下列字段（即使为空也要存在）：
  ...
  - `citation_analysis_path`（输出文件路径；文件内容为 UTF‑8 JSON 对象，仅 Introduction）
...
```json
{
  ...
  "citation_analysis_path": "C:\\\\Zotero\\\\storage\\\\XXXX\\\\citation_analysis.json",
  "provenance": {
...
2) 探测 `source_path` 内容格式：优先 PDF 签名，其次 UTF‑8 文本
3) 无论输入格式为何，统一生成 `<cwd>/.literature_digest_tmp/source.md`
4) 计算 `input_hash = sha256(file_bytes)`（以原始 `source_path` 文件内容为准）
...
- 检查 `digest_path`/`references_path`/`citation_analysis_path` 对应的文件是否存在，可读。
...
skill 输出的 `digest_path`/`references_path` 与 `source_path`/`language` 的组合共同确保幂等；Zotero 插件可根据调用时的输入再次对齐。
...
```json
{
  ...
  "warnings": [],
  "error": { "code": "FILE_NOT_FOUND", "message": "source_path not found" }
}
```

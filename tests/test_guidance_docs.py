...
class GuidanceDocsTests(unittest.TestCase):
    ...
    def test_skill_md_stays_slim_but_indexes_full_guidance(self):
        ...
        self.assertIn("`core_instruction`", skill_md)
        self.assertIn("`command_example`", skill_md)
        self.assertIn("repair 路径时，才额外参考 `sql_examples`", skill_md)
        self.assertIn("**最终 assistant 输出必须是一个 JSON 对象", skill_md)
        self.assertIn("不要直接写 SQLite 表来伪造阶段完成", skill_md)
        self.assertIn("\"digest_path\": \"/abs/path/digest.md\"", skill_md)
        self.assertIn("\"code\": \"normalize_source_failed\"", skill_md)
        self.assertIn("prepare_references_workset", skill_md)
        self.assertIn("persist_reference_entry_splits", skill_md)
        self.assertIn("review_reference_quality", skill_md)
        self.assertIn("quality_directives", skill_md)
        ...
        self.assertIn("persist_render_templates", skill_md)
        self.assertIn("`digest_template_path`", skill_md)
        self.assertIn("`citation_analysis_template_path`", skill_md)
        self.assertIn("从 prompt 主要语言推断", skill_md)
        ...
        self.assertIn("`split_mode`", skill_md)
        self.assertIn("`suspect_blocks`", skill_md)
        self.assertIn("`is_key_reference`", skill_md)
        self.assertIn("`selected_pattern`", skill_md)
        self.assertIn("### 1. `confirm_runtime_paths`", skill_md)
        self.assertIn("### 2. `bootstrap_runtime_db`", skill_md)
        self.assertIn("### 3. `persist_render_templates`", skill_md)
        self.assertIn("### 15. `render_and_validate --mode render`", skill_md)
        self.assertIn("`working_dir`", skill_md)
        self.assertIn("`result_json_path`", skill_md)
        self.assertIn("`cwd()` / `pwd`", skill_md)
        self.assertIn("单文件 `.tex`", skill_md)
        self.assertIn("LaTeX 工程目录", skill_md)
        self.assertIn("```bibtex", skill_md)
        ...
        self.assertNotIn("本步最常见错误", skill_md)
    ...
    def test_runtime_docs_preserve_guidance_without_history_noise(self):
        ...
        self.assertIn("## `confirm_runtime_paths`", stage_interface)
        self.assertIn("## `bootstrap_runtime_db`", stage_interface)
        self.assertIn("## `persist_render_templates`", stage_interface)
        self.assertIn("单文件 `.tex` / LaTeX 工程目录", stage_interface)
        self.assertIn("## `prepare_references_workset`", stage_interface)
        self.assertIn("## `persist_reference_entry_splits`", stage_interface)
        self.assertIn("## `review_reference_quality`", stage_interface)
        ...
        self.assertIn("`core_instruction: string`", gate_interface)
        self.assertIn("`command_example: object|null`", gate_interface)
        self.assertIn("`quality_directives: object|null`", gate_interface)
        self.assertIn("stage4_reference_quality", gate_interface)
        self.assertIn("## `core_instruction` 约束", gate_interface)
        self.assertIn("## `execution_note` 约束", gate_interface)
        self.assertIn("## `command_example` 约束", gate_interface)
        self.assertIn("failure_recovery.md", gate_interface)
        self.assertIn("receipt 链是否完整", gate_interface)
        self.assertIn("action_receipts.prepare_citation_workset", gate_interface)
        self.assertIn("repair 阶段固定返回 `null`", gate_interface)
        self.assertIn("非 repair 阶段固定返回空数组 `[]`", gate_interface)
        self.assertIn("\"command_example\": {", gate_interface)
        self.assertIn("review_reference_quality", gate_interface)
        ...
        self.assertIn("LaTeX 工程目录", step_01)
        self.assertIn("```bibtex", step_01)
        self.assertIn("可选物化副产物", step_01)
        self.assertIn("confirm_runtime_paths` 是唯一允许确定 `working_dir`", step_01)
        self.assertIn("bootstrap_runtime_db` 是唯一允许确定 `source_path`", step_01)
        self.assertIn("persist_render_templates` 是唯一允许确定 `digest_template_path`", step_01)
        self.assertIn("output_dir", step_01)
        self.assertIn("result_json_path", step_01)
        self.assertIn("不要先 `cd`", step_01)
        self.assertIn("先用用户显式指定值，否则从 prompt 主要语言推断", step_01)
        ...
        self.assertIn("非法 payload 示例", step_03)
        self.assertNotIn("非法旧 payload 示例", step_03)
        ...
        self.assertIn("`\\bibitem{...}` splitting", step_04)
        self.assertIn("`bibtex` splitting", step_04)
        self.assertIn("requires_split_review", step_04)
        self.assertIn("suspect_blocks", step_04)
        self.assertIn("line-first", step_04)
        self.assertIn("split", step_04)
        self.assertIn("merge", step_04)
        ...
        self.assertIn("Reference quality gate", step_04)
        self.assertIn("quality_directives", step_04)
        self.assertIn("bare_identifier_or_url_title", step_04)
        self.assertIn("review_reference_quality", step_04)
        self.assertIn("accept_warning", step_04)
        ...
        self.assertIn("first-author surname aliases", step_05)
        self.assertIn("\\cite{...}", step_05)
        self.assertIn("citekey_hint", step_05)
        self.assertIn("`citekey` / `bibitem_key`", step_05)
        self.assertIn("Waqas Zamir, S.", step_05)
        self.assertIn("citation_mentions_not_found", step_05)
        ...
        self.assertIn("不能靠手工写 SQLite", step_05)
        self.assertNotIn("允许输出空 `items=[]`", step_05)
        self.assertIn("citation_analysis.json.summary", step_06)
        self.assertIn("\"digest_path\": \"/abs/path/digest.md\"", step_06)
        self.assertIn("\"code\": \"citation_report_failed\"", step_06)
        ...
        self.assertIn("runtime_inputs.output_dir", step_06)
        self.assertIn("literature-digest.result.json", step_06)
        ...
        self.assertIn("本步只接受 `--out-md`、`--out-meta`、`--persist-db-only`", stage_interface)
        self.assertIn("## `prepare_references_workset`", stage_interface)
        self.assertIn("## `persist_reference_entry_splits`", stage_interface)
        self.assertIn("## `prepare_citation_workset`", stage_interface)
        self.assertIn("`\\bibitem` 起点切条", stage_interface)
        self.assertIn("`@type{key,` 起点切条", stage_interface)
        self.assertIn("`\\cite{...}`", stage_interface)
        ...
        self.assertIn("action receipts", stage_interface)
        ...
        self.assertIn("当前 `citation_items` 中找到", stage_interface)
        self.assertIn("[AY-k]", stage_interface)
        self.assertIn("关键文献", stage_interface)
        self.assertIn("本步只接受 `--out` 与 `--persist-db-only`", stage_interface)
        self.assertIn("--output-dir", stage_interface)
        self.assertIn("runtime_inputs.output_dir", stage_interface)
        self.assertIn("runtime_inputs.result_json_path", stage_interface)
        self.assertIn("runtime_inputs.digest_template_path", stage_interface)
        self.assertIn("runtime_inputs.citation_analysis_template_path", stage_interface)
        self.assertIn("literature-digest.result.json", stage_interface)
        self.assertIn("所有文件路径字段都输出为绝对路径", stage_interface)
        self.assertIn("核心执行指令", stage_interface)
        self.assertIn("\"provenance\": {", stage_interface)
        ...
        self.assertIn("split_mode", stage_interface)
        self.assertIn("suspect_blocks", stage_interface)
        self.assertIn("suspicion_kind", stage_interface)
        self.assertIn("split", stage_interface)
        self.assertIn("keep", stage_interface)
        self.assertIn("merge", stage_interface)
        ...
        self.assertNotIn("旧的 `sections[]`", step_03)
        self.assertNotIn("非法旧接口", step_03)
        self.assertNotIn("旧字段", stage_interface)
        self.assertNotIn("旧字段迁移", step_06)
        dev_doc = self.read("docs/dev_paper_digest_skill.md")
        self.assertNotIn("guidance_migration_map.md", dev_doc)
        self.assertNotIn("legacy_snapshot", dev_doc)
    ...
    def test_templates_preserve_original_intent(self):
        ...
        citation_en = self.read("literature-digest/assets/templates/citation_analysis.en-US.md.j2")
        citation_zh = self.read("literature-digest/assets/templates/citation_analysis.zh-CN.md.j2")

        self.assertIn("## 研究问题与贡献", digest_zh)
        self.assertIn("## Research Question & Contributions", digest_en)
        self.assertIn("Key References", citation_en)
        self.assertIn("Timeline Analysis", citation_en)
        self.assertIn("Keywords", citation_en)
        self.assertIn("总体总结", citation_zh)
        self.assertIn("关键文献", citation_zh)
        self.assertIn("按功能归类", citation_zh)
        self.assertIn("时间线分析", citation_zh)
    ...
    def test_runner_points_to_full_external_guidance(self):
        ...
        self.assertIn("Also obey the fixed `core_instruction` and the short `execution_note` returned by gate", runner)
        self.assertIn("gate-returned `core_instruction`", runner)
        ...
        self.assertIn("confirm_runtime_paths", runner)
        self.assertIn("DB-stored paths", runner)
        self.assertIn("single `.tex` file", runner)
        self.assertIn("LaTeX project directory", runner)
    ...
    def test_glossary_becomes_shared_terminology_source(self):
        ...
        self.assertIn("`output_dir`", skill_md)
